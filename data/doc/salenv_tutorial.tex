...
All SALenv tools search for context files in the path specified by the 
environment variable \verb.SALPATH..
If you are using \verb.bash., then you should modify your \verb|.bashrc| file. 
For instance, to set \verb.SALPATH. to the current and the \verb./homes/leonardo/tmp. directories,
you should add the following command to your \verb|.bashrc| file:

\begin{session}
export SALPATH=.:/homes/leonardo/tmp
\end{session}

If you are using \verb.tcsh., then you should modify your \verb|.cshrc| file:

\begin{session}
setenv SALPATH .:/homes/leonardo/tmp
\end{session}

If the environment variable \verb.SALPATH. is not specified, then the tools will only  
...
SALenv contains a simulator for finite state specifications based on BDDs. The simulator allows users 
...
\verb|short.sal|, the current directory is in the \verb.SALPATH., and
...
\begin{session}
\% sal-sim
SAL Simulator (Version 2.1). Copyright (c) 2003, 2004 SRI International.
Build date: Wed Jan 21 11:34:54 PST 2004
Type `(exit)' to exit.
Type `(help)' for help.
